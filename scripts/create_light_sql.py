...
ACCOUNT_CONSTRAINTS = re.compile(r'ALTER TABLE ONLY (?P<table>public\.\w+)\s+ADD CONSTRAINT (?P<constraint>["\w]+) FOREIGN KEY \((?P<fk>\w+)\) REFERENCES (?P<fk_table>public\.\w+)\((?P<field>\w+)\)(?P<tail>[^;]*);')
...
TABLE_RE = re.compile('CREATE TABLE public\.(\w+) ')
...
def main():
    ...
    PARTIAL_TABLES = [
        ('account_user', "email NOT LIKE '%@okfn.de' OR private = TRUE OR email IS NULL; UPDATE account_user SET password='';"),
        ('document_document', "user_id IS NULL;"),
        ('document_document', "id NOT IN (SELECT id FROM table ORDER BY id DESC LIMIT 500);"),
