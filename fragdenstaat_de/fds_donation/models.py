...
INTERVAL_CHOICES_DICT = dict(INTERVAL_CHOICES)
...
class DonationProgressBarCMSPlugin(CMSPlugin):
    ...
    initial_amount = models.DecimalField(
        decimal_places=2, max_digits=10, blank=True, default=decimal.Decimal
    )
...
class CheckDonorTagCMSPlugin(CMSPlugin):
    ...
    def __str__(self):
        if self.negate:
            return _("Donor doesn't have tag '{tag}'").format(tag=self.tag)
        else:
            return _("Donor has tag '{tag}'").format(tag=self.tag)
