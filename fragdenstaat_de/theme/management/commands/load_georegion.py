...
class Command(BaseCommand):
    ...
    def create_hierarchy(self):
        ...
        for small, big in matches:
            for small_obj in GeoRegion.objects.filter(kind=small, part_of__isnull=True):
                print("Trying", small_obj)
                try:
                    big_objs = GeoRegion.objects.filter(
                        kind=big, geom__covers=small_obj.geom.point_on_surface
                    )
                except GEOSException:
                    big_objs = GeoRegion.objects.filter(
                        kind=big, geom__covers=small_obj.geom.centroid
                    )
                if not big_objs:
                    big_objs = GeoRegion.objects.filter(
                        kind=big, geom__intersects=small_obj.geom
                    )
                if len(big_objs) == 1:
                    ...
                    big_one = [
                        x
                        for x in big_objs
                        if small_obj.region_identifier.startswith(x.region_identifier)
                    ]
                    if len(big_one) == 1:
