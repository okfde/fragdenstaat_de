...
class Command(BaseCommand):
    ...
    def import_page(self, row):
        ...
        page = create_page(
            title_str, 'cms/help_base.html', 'de',
            parent=top_page, in_navigation=True,
            slug=slugify(title_str, replacements=[['ä', 'ae'], ['ö', 'oe'], ['ü', 'ue']]),
            position='last-child',
            **kwargs
        )
        placeholder = page.placeholders.get(slot='content')
        add_plugin(placeholder, 'TextPlugin', 'de',
            body='''<h2>{}</h2><p>Inhalt folgt...</p>'''.format(
                title_str
            )
        )
